...
namespace YGDR.MDV
{
    ...
    public class BlockContainer : Block
    {
        ...
        public void CollectMatches( string query, List<List<Rect>> results, Context context )
        {
            foreach( var block in mBlocks )
            {
                if( block is BlockContent bc )          bc.CollectMatches( query, results, context );
                else if( block is BlockContainer bk )   bk.CollectMatches( query, results, context );
                else if( block is BlockCollapsible bl ) bl.CollectMatches( query, results, context );
            }
        }
    ...
    public class BlockContent : Block
    {
        ...
        public void CollectMatches( string query, List<List<Rect>> results, Context context )
        {
            ...
            var itemStart = new List<int>();
            var items     = new List<Content>();

            foreach( var c in mContent )
            {
                if( !( c is ContentText ) || string.IsNullOrEmpty( c.Payload.text ) ) continue;
                itemStart.Add( sb.Length );
                items.Add( c );
                sb.Append( c.Payload.text );
            }
            ...
            while( ( qi = full.IndexOf( query, qi, System.StringComparison.OrdinalIgnoreCase ) ) >= 0 )
            {
                var matchEnd   = qi + query.Length;
                var matchRects = new List<Rect>();

                for( var i = 0; i < items.Count; i++ )
                {
                    var cStart = itemStart[ i ];
                    var cEnd   = cStart + items[ i ].Payload.text.Length;

                    if( cEnd <= qi || cStart >= matchEnd ) continue;

                    var localStart = Mathf.Max( qi, cStart ) - cStart;
                    var localEnd   = Mathf.Min( matchEnd, cEnd ) - cStart;
                    var text       = items[ i ].Payload.text;

                    var style   = context.Apply( items[ i ].Style );
                    var hPad    = style.padding.left + style.padding.right;
                    var prefixW = localStart > 0 ? Mathf.Max( 0f, style.CalcSize( new GUIContent( text.Substring( 0, localStart ) ) ).x - hPad ) : 0f;
                    var matchW  = Mathf.Max( 2f, style.CalcSize( new GUIContent( text.Substring( localStart, localEnd - localStart ) ) ).x - hPad );

                    matchRects.Add( new Rect( items[ i ].Location.x + style.padding.left + prefixW, items[ i ].Location.y, matchW, items[ i ].Location.height ) );
                }

                if( matchRects.Count > 0 ) results.Add( matchRects );
                qi = matchEnd;
            }
        }
    ...
    public class BlockCollapsible : Block
    {
        ...
        public void CollectMatches( string query, List<List<Rect>> results, Context context )
            => Content.CollectMatches( query, results, context );
